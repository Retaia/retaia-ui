...
export function t(locale: Locale, key: MessageKey | string, params?: Params) {
  const localized = messages[locale][key as MessageKey]
  const english = messages.en[key as MessageKey]
  const resolved = localized ?? english ?? key
