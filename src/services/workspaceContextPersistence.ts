type WorkspaceScope = 'review' | 'library'
...
export function persistSelectedAssetId(scope: WorkspaceScope, assetId: string | null) {
  updateContext(scope === 'review' ? { reviewSelectedAssetId: assetId } : { librarySelectedAssetId: assetId })
}

export function readSelectedAssetId(scope: WorkspaceScope): string | null {
  const value = scope === 'review' ? readContext().reviewSelectedAssetId : readContext().librarySelectedAssetId
...
export function persistScrollY(scope: WorkspaceScope, scrollY: number) {
  ...
  updateContext(scope === 'review' ? { reviewScrollY: value } : { libraryScrollY: value })
}

export function readScrollY(scope: WorkspaceScope): number {
  const value = scope === 'review' ? readContext().reviewScrollY : readContext().libraryScrollY
