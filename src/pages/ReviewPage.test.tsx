...
describe('App', () => {
  ...
  it('does not render auth controls in review page', async () => {
    const previous = import.meta.env.VITE_ASSET_SOURCE
    try {
      import.meta.env.VITE_ASSET_SOURCE = 'api'
      vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      ...
      expect(screen.queryByLabelText('Authentification utilisateur')).not.toBeInTheDocument()
      expect(screen.queryByTestId('api-connection-save')).not.toBeInTheDocument()
    } finally {
      import.meta.env.VITE_ASSET_SOURCE = previous
      vi.restoreAllMocks()
    }
